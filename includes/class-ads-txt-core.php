...
class Ads_Txt_Core {
	...
	private function write_file_to_root( $filename, $content ) {
		...
		if ( empty( $wp_filesystem ) || ! is_object( $wp_filesystem ) ) {
			...
			if ( ! WP_Filesystem() || ! is_object( $wp_filesystem ) ) {
				return new WP_Error( 'filesystem_failed', __( 'WordPress failed to initialize filesystem. Using fallback routing instead.', 'monetiscope-ads-txt-inserter' ) );
			}
		}
	...
	public function restore_backup( $type, $timestamp ) {
		...
		return new WP_Error( 'backup_not_found', __( 'Specified backup not found.', 'monetiscope-ads-txt-inserter' ) );
	}
