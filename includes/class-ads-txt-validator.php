		...
		foreach ( $lines as $index => $line_raw ) {
			...
			if ( isset( $seen[ $normalized ] ) ) {
				...
				$results['warnings'][] = array(
					...
					'message' => sprintf( __( 'Duplicate entry found on line %1$d (matches line %2$d).', 'monetiscope-seller-records' ), $line_num, $seen[ $normalized ] ),
				);
				continue;
			}
			...
			if ( $count < 3 || $count > 4 ) {
				$results['errors'][] = array(
					...
					'message' => sprintf( __( 'Invalid formatting on line %d. Expected 3 or 4 comma-separated values (Domain, Publisher ID, Relationship, optional Cert Authority ID).', 'monetiscope-seller-records' ), $line_num ),
				);
				$results['valid'] = false;
				continue;
			}
			...
			if ( ! self::is_valid_domain( $domain ) ) {
				$results['errors'][] = array(
					...
					'message' => sprintf( __( 'Invalid domain format "%1$s" on line %2$d.', 'monetiscope-seller-records' ), esc_html( $domain ), $line_num ),
				);
				$results['valid'] = false;
			}
			...
			if ( empty( $pub_id ) ) {
				$results['errors'][] = array(
					...
					'text' => $line,
					/* translators: %d: line number */
					'message' => sprintf( __( 'Missing publisher/seller ID on line %d.', 'monetiscope-seller-records' ), $line_num ),
				);
				$results['valid'] = false;
			}
			...
			if ( $relation !== 'DIRECT' && $relation !== 'RESELLER' ) {
				$results['errors'][] = array(
					...
					'message' => sprintf( __( 'Invalid relationship type "%1$s" on line %2$d. Must be DIRECT or RESELLER (case insensitive).', 'monetiscope-seller-records' ), esc_html( $relation ), $line_num ),
				);
				$results['valid'] = false;
			}
		}
